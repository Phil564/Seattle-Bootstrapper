...
void Bootstrapper::RegisterUninstall(const TCHAR *productName)
{
	...
	throwHRESULT (keyProductCode->SetStringValue(_T("Publisher"), _T("Seattle Corporation")), "Failed to set Publisher key");
	throwHRESULT (keyProductCode->SetStringValue(_T("URLInfoAbout"), _T("http://leah.rocks")), "Failed to set URLInfoAbout key");
		...
		if(moduleVersionNumber != bootstrapperVersion){
			...
			catch (...)
			{
				newBootstrapper = simple_logger<wchar_t>::get_temp_filename(_T("tmp"));		// We could use an "exe" extension, but hiding the type isn't a bad idea?
				{
					...
					std::string eTag;
					HttpTools::httpGetCdn(this, installHost, format_string("/%s-Seattle.exe", installVersion.c_str()), eTag, bootstrapperFile, false, boost::bind(&Bootstrapper::dummyProgress, _1, _2));
				}
			}
...
void Bootstrapper::setLatestProcess()
{
	...
	const std::string latestProcessName = format_string("www.leah.rocks/%s/%s/latestProcess", installHost.c_str(), Type().c_str());
