...
std::shared_ptr<CRegKey> CreateKey(HKEY parent, const TCHAR* name, const TCHAR* defaultValue, bool is64bits)
{  
   std::shared_ptr<CRegKey> key = std::make_shared<CRegKey>();
...
{
	CString desc(e.Description().GetBSTR());
	CString errorMessage(e.ErrorMessage());
	CString source(e.Source().GetBSTR());

	std::ostringstream message;
	message << "_com_error stage=" << stage
		<< " error=" << e.Error()
		<< " message='" << CT2A(errorMessage.GetString()) << "'"
		<< " desc='" << CT2A(desc.GetString()) << "'"
		<< " source='" << CT2A(source.GetString()) << "'";
